...
- When you specify a standard deviation in any of the investment growth parameters (ETFs, investment trusts and private pension), the simulator runs the scenario a large number of times instead of one, randomly varying the growth of the investments using a gaussian distribution with the mean and variance you specified, and count the number of runs in which the scenario was successful (it reached your target age without running out of money). At the end it will display the percentage of times that the simulation ended with success.
