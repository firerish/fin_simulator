...
Please let me know if you find any issues. I'm [/u/firerish](http://www.reddit.com/u/firerish) on reddit. If you want to contribute, the code of this simulator is open source: https://github.com/firerish/fin_simulator*
											
### Introduction

The main point of this exercise is to experiment with different ways to use your money and see how they play out in your retirement. You can let it sit in your bank account and lose buying power to inflation, or you may invest it in real estate, in the stock market, or in pension funds. This simulator assumes a constant yearly growth rate for things like inflation and other factors, but you can introduce volatility for your investments in which case it will run a [Monte Carlo simulation](https://www.investopedia.com/terms/m/montecarlosimulation.asp). You specify the rates of growth so you can see what happens if inflation is higher, investments grow slower or are more volatile, etc. I recommend entering conservative figures. Use the worst plausible growth rates and add a stock market crash or two to stress test your scenarios. Also note that I'm not using the usual "safe withdrawal rate". Instead, the simulation takes as much as it needs from various funds to pay for the specified expenses. If it runs out of money, the scenario failed.

### Before you start

- Before you do anything else, make your own copy of this spreadsheet. You won't be able to run your own scenarios otherwise.
- I plan to fix bugs and add new features for some time. Come back [here](https://docs.google.com/spreadsheets/d/1aPClS0_Fi4x14zIkQORALhZeFEeWw611Z-rrXYvy5Ko/edit?usp=sharing) every once in a while to check the version (and the change log at the bottom of this page).
...
- **State pension weekly**: The state pension is paid weekly and depends on how many periods you contributed to it. Read about it [here](https://www.citizensinformation.ie/en/social_welfare/social_welfare_payments/older_and_retired_people/state_pension_contributory.html).
...
- **Trust growth**: This is the expected growth (by specifying average and volatility) for investment trusts, individual shares, or any asset that is taxed at 33% on exit gains. Again dividends are not considered as a separate event and are assumed to be reinvested. You can read more about investment options [here](https://mrsmoneyhacker.com/investment-options-in-ireland/).
- **Drawdown Priorities**: Once in retirement you will start to draw down on your investments. Here you decide where to take money from first (priority 1), then where to take it from after the first choice is depleted (priority 2), and so on.
...
1. **Event spec**: Specifies the event type and name. It has to follow the format <*type*>:<*name*> (for example "E:college").
   - Type must be one of **SI** (salary income), **RI** (rent income), **UI** (RSU income), **FI** (Tax-free Income), **E** (expense), **R** (real estate), **M** (mortgage), **SM** (stock market).
...
- **Tax-free Income event (FI)**: This event lets you add a net income from things like a gift or inheritance. It assumes any tax has already been paid, so this is kind of a wildcard event that lets you inject cash into your income stream, and it will be invested according to your chosen allocation.
...
**About the Monte Carlo mode**
- When you specify a standard deviation in any of the investment growth parameters (ETFs, investment trusts and private pension), the simulator runs the scenario a large number of times, randomly varying the growth of the investments using a gaussian distribution with the mean and variance you specified, and it counts the number of runs in which the scenario was successful (it reached your target age without running out of money). At the end it will display the percentage of times that the simulation ended in success.
...
- 1.22: Added tax-free income event to take care of things like gifts or inheritance (contributed by Lukasz Sanek).
